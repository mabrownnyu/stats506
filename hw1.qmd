...
```{r}
df <- df[!is.na(df$state) & df$state != "XX", ]
```

I removed all records where state is not reported or where the state is "XX".
...
# filter out NAs
df <- df[!is.na(df$food_spending) & !is.na(df$grocery_spending)
         & !is.na(df$dining_spending & !is.na(df$misc_spending)), ]
